...
import (
	"fmt"
	"package/rectangle" //Прописываем путь до пакета rectangle
) //0. Разделяющая область видимости уровня одного пакета
//1. В этом примере 2 модуля (main и calculator) помещены в одну директорию package
//2. Говорят, что модули main и calculator находятся в одном пакете (в одной директории)

//3. Разделяющая область видимости
// Все, что в принципе находится внутри данного пакета - доступно из любого модуля без импортирования.
