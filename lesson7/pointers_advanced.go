...
func selfMutation(selfSlice *[3]int) {
	// (*selfSlice)[1] = 1005
	selfSlice[1] = 1005
}
...
func main() {
	...
	selfMutation(&arr)
	fmt.Println("Arr after selfMutation:", arr)

	selfArray := [5]int{10, 11, 12, 13}
	fmt.Printf("Self array type as [:] : %T \n", selfArray[:])
	// mutationSlice([selfArray[:]])

	selfSlice := []int{1, 2, 3, 4, 5, 7, 8}
	mutationSlice(selfSlice)
	fmt.Println(selfSlice[:])
	fmt.Printf("Self array type as [:] : %T \n", selfSlice[:2])
